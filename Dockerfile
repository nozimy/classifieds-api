...
RUN /etc/init.d/postgresql start &&\
    psql --command "CREATE USER avito WITH SUPERUSER PASSWORD 'avito';" &&\
    createdb -O avito avito &&\
...
COPY --from=build /usr/src/app/classifieds-api .

#
# Запускаем PostgreSQL и сервер
#
CMD service postgresql start && ./classifieds-api --port=5000
